    ...
    for idx, path in enumerate(paths):
        ...
        try:
            ...
            if img_mode == 'RGBA':  # RGBA images should be saved in png format
                extension = 'png'
            if suffix == '':
                save_path = os.path.join('Face_img', f'{imgname}.{extension}')
            else:
                save_path = os.path.join('Face_img', f'{imgname}_{suffix}.{extension}')
            
            # 결과 저장
            print("원본이 사라지고 결과가 저장됩니다.")
            os.remove(save_path) # 원본 제거 
            cv2.imwrite(save_path, output) # esr gan 결과 저장 